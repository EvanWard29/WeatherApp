...
{
    public partial class frmMaxTemp : Form
    {
        public static frmMaxTemp frmMaxGraph = null;
        public frmMaxTemp()
        {
            InitializeComponent();
            frmMaxGraph = this;
        ...
        private void frmMaxTemp_Load(object sender, EventArgs e)
        {
            loadLocations();
        }
        ...
        public void loadDetails()
        {
                ...
                if (this.drpLocation.SelectedItem.ToString() == l.getLocationName())
                {
                    ...
                    foreach(Year y in years)
                    {
                        if (this.txtYear.Text == y.GetYear().ToString())
                        {
                            ...
                            {
                                double temp = m.GetMaxTemp();

                                if(temp > maxTemp)
                                {
                                    maxTemp = temp;
                                    hotMonthID = m.GetMonthID() - 1;
                                }
            ...
            string hotMonth = months[hotMonthID];
            showDetails(hotMonth, maxTemp, minTemp, tempValues);
        }

        public void showDetails(string hotMonth, double maxTemp, double minTemp, double[] tempValues)
        {
            this.txtLocation.Text = this.drpLocation.SelectedItem.ToString();
            this.txtYearID.Text = this.txtYear.Text;
            this.txtHotMonth.Text = hotMonth;
