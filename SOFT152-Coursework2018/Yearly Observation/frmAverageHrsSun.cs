...
{
    public partial class frmAverageHrsSun : Form
    {
        public static frmAverageHrsSun frmSun = null;
        double[] avgValues;

        public frmAverageHrsSun()
        {
            InitializeComponent();
            frmSun = this;
        }
        ...
        private void frmAverageHrsSun_Load(object sender, EventArgs e)
        {
            loadLocations();
        }
                ...
                if (this.drpLocation.SelectedItem.ToString() == l.getLocationName())
                {
                    ...
                    if (years != null)
                    {
                        foreach (Year y in years)
                        {
                            ...
                            {
                                double tempValue = m.GetHrsOfSun();
        ...
        {
            int maxSunIndex = Array.IndexOf(avgValues, avgValues.Max());

            this.txtLocation.Text = this.drpLocation.SelectedItem.ToString();
            this.txtSunYear.Text = Years[maxSunIndex] + ":\n" + avgValues.Max() + " Hrs";
        ...
        public void drawYAxis()
        {
            ...
            Font arial3 = new Font(new FontFamily("Arial"), 6, style3);

            double maxTemp = Math.Round(avgValues.Max()) + 8;
            ...
            for (int i = 0; i < number; i++)
            {
                if (i % 10 == 0)
                {
                    yAxis.DrawString(maxTemp.ToString() + " Hrs", arial3, brush, x, y);
                }
                maxTemp = maxTemp - 1;
        ...
        public void drawGraph()
        {
            ...
            for (int i = 0; i < avgValues.Length; i++)
            {
                ...
                graph.DrawString("*".ToString(), arial3, brush, x, Convert.ToInt32(y + 8));
                x = x + length + gap;
            }
        }
    }
